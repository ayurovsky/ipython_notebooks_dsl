...
def run_table_generate_pipeline(file_prefix, input_path, output_path, SD): 
    ...
    out_file_prefix = output_path + file_prefix
    try:
        ret = subprocess.check_output(["perl", "alisa_efficient_codon_pair_score_hexamers_paper.pl", "-s", \
                                       processed_cds_file, "-o", out_file_prefix, "-z", "300"], stderr=subprocess.STDOUT)
    except subprocess.CalledProcessError as exc:
...
csv_with_table_stats_added = 'Diwan_MissingBacteria_Files_WithAllStats_only_tompa_v4.csv'
